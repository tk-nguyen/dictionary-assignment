...
public class DictionaryManagement
{
	...
	/*
	 * Đọc dữ liệu từ file dictionaries.txt Target và meaning ngăn cách bởi tab Sử
	 * dụng String.split("\\s+") để tách String thành String[], bỏ tất cả khoảng
	 * trắng giữa từ và nghĩa
	 */
	...
	public boolean fix(String s, int n, String fix)
	{
		...
		for (int i = 0; i < loadedDatabase.size(); i++)
		{
			if (s.equals(loadedDatabase.get(i).getWord_target()))
			{
				...
				{
				case 1:
					loadedDatabase.get(i).setWord_target(fix);
					break;
				case 2:
					loadedDatabase.get(i).setWord_pronounce(fix);
					break;
				case 3:
					loadedDatabase.get(i).setWord_explain(fix);
					break;
				}
				isFix = true;
				isInLoadedDB = true;
		...
		if (!isInLoadedDB)
		{
			for (int i = 0; i < addToDatabase.size(); i++)
			{
				if (s.equals(addToDatabase.get(i).getWord_target()))
				{
					...
					{
					case 1:
						addToDatabase.get(i).setWord_target(fix);
						break;
					case 2:
						addToDatabase.get(i).setWord_pronounce(fix);
						break;
					case 3:
						addToDatabase.get(i).setWord_explain(fix);
						break;
					}
					isFix = true;
					break;
